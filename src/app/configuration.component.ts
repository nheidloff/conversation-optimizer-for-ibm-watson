...
export class ConfigurationComponent {
	...
	readWorkspaces(): void {
		...
		if (workspacesInStore.length < 1) {
			this.ngRedux.dispatch({ type: ToolAction.SET_USER_INTERFACE_DISABLED, payload: { disabled: true, text: 'Reading Workspaces' } });
			this.getWorkspaceList((err, workspaces) => {
				if (err) {
					console.log(err);
					this.ngRedux.dispatch({ type: ToolAction.SET_WATSON_CONVERSATION_CONFIG_STATUS, payload: Tool.WATSON_CONVERSATION_CONFIG_STATUS_INVALID });
				}
				else {
					...
					this.ngRedux.dispatch({ type: ToolAction.SET_WATSON_CONVERSATION_CONFIG_STATUS, payload: Tool.WATSON_CONVERSATION_CONFIG_STATUS_VALIDATED });
				}
				this.ngRedux.dispatch({ type: ToolAction.SET_USER_INTERFACE_DISABLED, payload: { disabled: false, text: '' } });
			});
		}
	}
	...
	onBlurWatsonPassword(event) {
		//if ((this.ngRedux.getState().tool.watsonConversationConfigStatus == Tool.WATSON_CONVERSATION_CONFIG_STATUS_COMPLETE) ||
		//	(this.ngRedux.getState().tool.watsonConversationConfigStatus == Tool.WATSON_CONVERSATION_CONFIG_STATUS_VALIDATED) ||
		//	(this.ngRedux.getState().tool.watsonConversationConfigStatus == Tool.WATSON_CONVERSATION_CONFIG_STATUS_CREDENTIALS_SYNTACTICALLY)) {
		//	this.readWorkspaces();
		//}
	}
	...
	readLogs(): void {
		Common.readLogs(this.ngRedux, this.http, (err) => {
			if (!err) this.router.navigate(['/optimize']);
		})
	}
}